    ...
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      ...
      <section className="mb-10">
        ...
        <div className="flex items-center gap-6 mt-5">
          <div className="text-center">
            <div className="text-xl font-bold text-lobster-500">{publishedSeries.length + 1}</div>
      ...
      {publishedSeries.length === 0 ? (
        ...
        <div className="grid gap-6">
              ...
              >
                ...
                <div className={`relative bg-gradient-to-br ${coverColor} sm:w-44 h-36 sm:h-auto flex-shrink-0 flex flex-col items-center justify-center gap-2`}>
                  ...
                  <div className="flex items-center gap-1 bg-black/20 backdrop-blur-sm rounded-full px-2.5 py-1 text-white/90 text-xs font-medium">
                    ...
                    {publishedPosts.length > 0 && <span>{publishedPosts.length} 篇</span>}
                    {reportsCount > 0 && <span>{reportsCount} 报告</span>}
                    {totalReadTime > 0 && (
                      <>
                    ...
                    return (
                      <div className="border-t border-gray-100 dark:border-gray-800 pt-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          {/* 报告文件：每条单独展示，可点击（无报告时不显示空行） */}
                          {reportsCount > 0 && (series.reports || []).map(r => {
          ...
          {/* 外部专题：智能体设计模式（与系列卡片同构） */}
          <a
            href="/agentic-design-patterns/"
            className="group flex flex-col sm:flex-row rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1a1d27] hover:border-lobster-300 dark:hover:border-lobster-700 hover:shadow-xl dark:hover:shadow-lobster-900/10 transition-all duration-300"
          >
            {/* Cover */}
            <div className="relative bg-gradient-to-br from-green-400 to-green-600 sm:w-44 h-36 sm:h-auto flex-shrink-0 flex flex-col items-center justify-center gap-2">
              <span className="text-5xl group-hover:scale-110 transition-transform duration-300 drop-shadow">
                🧩
              </span>
              {/* 统计徽章 */}
            ...
            <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between min-w-0">
              ...
              <div>
                ...
                {/* 简介：左边竖线 + 正文 */}
                <div className="flex gap-2.5 mb-4">
                  <div className="w-0.5 flex-shrink-0 rounded-full bg-gradient-to-b from-green-400 to-green-600 opacity-60 self-stretch min-h-[1em]" />
                  <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed min-w-0">
                    《Agentic Design Patterns》中英对照教程：21 种设计模式、42 篇章节内容、22 个代码示例与视频讲解，独立部署的知识专题站。
                  </p>
                </div>
                ...
                <div className="border-t border-gray-100 dark:border-gray-800 pt-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {[
                      ...
                      <div key={title} className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                        <span className="w-5 h-5 flex-shrink-0 rounded-full bg-gray-100 dark:bg-gray-800 text-xs flex items-center justify-center font-semibold text-gray-400 dark:text-gray-500">
                          {i + 1}
                        </span>
                        <span className="truncate group-hover:text-gray-700 dark:group-hover:text-gray-200 transition-colors">{title}</span>
                      </div>
                    ))}
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-400 dark:text-gray-500">
                      ...
                      <span className="truncate">图文教程 · 代码示例 · 视频讲解</span>
                      <span className="ml-auto flex-shrink-0 text-xs text-gray-300 dark:text-gray-600">42 篇</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-400 dark:text-gray-500">
                      <span className="w-5 h-5 flex-shrink-0 rounded-full bg-gray-100 dark:bg-gray-800 text-xs flex items-center justify-center">🌐</span>
                      <span className="truncate">独立部署知识专题站</span>
                      <span className="ml-auto flex-shrink-0 text-xs text-gray-300 dark:text-gray-600">外部</span>
                    </div>
                  </div>
                </div>
              </div>
